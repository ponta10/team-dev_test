...
if(!isset($_GET['search'])){
    $stmt = $pdo->prepare("SELECT * FROM agent_user JOIN agent ON agent.id = agent_user.agent_id RIGHT JOIN apply_info ON apply_info.id = agent_user.user_id where agent_name = '$agent' and date like '$like'");
    $stmt->execute();
    $cnts = $stmt->fetchAll();
}else{
    $stmt = $pdo->prepare("SELECT * FROM agent_user JOIN agent ON agent.id = agent_user.agent_id RIGHT JOIN apply_info ON apply_info.id = agent_user.user_id where agent_name = '$agent' and date like '$like' and graduate_year = '$search'");
    $stmt->execute();
    $cnts = $stmt->fetchAll();
}
...
$stmt_count = $pdo->prepare("SELECT count(agent_name) FROM agent_user JOIN agent ON agent.id = agent_user.agent_id RIGHT JOIN apply_info ON apply_info.id = agent_user.user_id where agent_name = '$agent' and date like '$like'");
$stmt_count->execute();
